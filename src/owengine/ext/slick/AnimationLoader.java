	...
	public static void init(String resDir) throws SlickException {
		for (MoveDir dir : new MoveDir[]{left, right})
			for (ActionType type : MovementActionType.values())
				animations.put(type+"_"+dir, loadAnimation(resDir, "move", dir, 2));
		for (MoveDir dir : new MoveDir[]{up, down}) {
			animations.put(turn+"_"+dir, loadAnimation(resDir, "move", dir, 2));
			for (ActionType type : new ActionType[]{bump, move})
				animations.put(type+"_"+dir, loadAnimation(resDir, "move", dir, 3));
		}
	}

	private static Animation loadAnimation(String resDir, String name, MoveDir dir, int frames)
			throws SlickException {
		Image[] images = new Image[frames];
		for (int i = 0; i < frames; i++)
			images[i] = new Image(resDir+"/pc/"+name+"/"+dir+"/"+i+".png");
